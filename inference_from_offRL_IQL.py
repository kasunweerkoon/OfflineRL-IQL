...
class OffRL_Inference:
	def __init__(self):
		...
		self.height_pub = rospy.Publisher("/height_map", sensor_msgs.msg.Image, queue_size=20) 
		...
		if choice_model ==1:
			...
			model_name = 'iql_sep13k_80'
		elif choice_model ==3:
			model_name = 'iql_sep13k_120'
		elif choice_model ==4:
			model_name = 'iql_sep13k_160'
		elif choice_model ==5:
			model_name = 'iql_sep13k_200'
		elif choice_model ==6:
			model_name = 'iql_sep13k_240'
	...
	def goal_callback(self, data):
		...
		self.goalX_odom =  self.current_position_xyz.x + goalX_rob*math.cos(self.yaw_rad) - goalY_rob*math.sin(self.yaw_rad)
		self.goalY_odom = self.current_position_xyz.y + goalX_rob*math.sin(self.yaw_rad) + goalY_rob*math.cos(self.yaw_rad)
	...
	def odometry_callback(self,odom_data):
		...
		if self.goal_received:
			...
			current_to_goal_distance = math.hypot(self.odom_goal[0] - current_odom[0], self.odom_goal[1] - current_odom[1])
			...
			states = current_state.to(self.device).float() #current_states_cloned.to(self.device).float()
			actions = actions.to(self.device).float() #actions_cloned.to(self.device).float() 
			...
			if self.pub_end2end_action == 1:
				if current_to_goal_distance <=0.75:
					self.pub_action.linear.x = 0
					self.pub_action.linear.y = 0
					self.pub_action.angular.z = 0
...
def main(args):
	ic = OffRL_Inference()
	rospy.init_node('offlineRL_IQL_inference', anonymous=True)
	try:
		rospy.spin()
